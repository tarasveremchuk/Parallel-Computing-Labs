...
public class InternalDeviceController {
    ...
    @GetMapping("/devices/accessible/{userId}")
    public ResponseEntity<List<String>> getAccessibleDeviceIds(@PathVariable UUID userId) {
        List<Device> owned = deviceRepository.findByOwnerIdAndDeletedFalse(userId);
        List<UUID> sharedIds = accessRepository.findDeviceIdsByUserId(userId);
        List<String> allIds = new ArrayList<>(owned.stream().map(d -> d.getId().toString()).toList());
        for (UUID id : sharedIds) {
            String idStr = id.toString();
            if (!allIds.contains(idStr)) allIds.add(idStr);
        }
